...
class TestPasswordRecover:
    ...
    def test_click_password_recover_button(self, driver):
        password_recovery_page = PasswordRecoverPage(driver)
        HeaderPage(driver).click_user_account_btn()
        UserAccountPage(driver).click_password_recover_btn()
        password_recovery_page.click_recover_btn()
        assert password_recovery_page.find_save_btn().is_displayed()
    ...
    def test_enter_email_and_click_recover(self, create_and_delete_user, driver):
        password_recovery_page = PasswordRecoverPage(driver)
        password_recovery_page.open_page(Urls.FORGOT_PASSWORD_PAGE)
        password_recovery_page.set_email_for_recover_password(create_and_delete_user[0]['email'])
        password_recovery_page.click_recover_btn()
        assert password_recovery_page.find_save_btn().is_displayed()
    ...
    def test_active_password_field(self, create_and_delete_user, driver):
        ...
        password_recovery_page.set_email_for_recover_password(create_and_delete_user[0]['email'])
        password_recovery_page.click_recover_btn()
        password_recovery_page.find_save_btn()
        password_recovery_page.click_on_show_password_icon()
        assert password_recovery_page.find_input_password_active()
